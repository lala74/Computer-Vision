...
void setGaussian(cv::Mat& g, float sigma, int halfSize)
{
    ...
    if(halfSize == -1) halfSize = max(1, (int)(4.0 * sigma));
...
void removeNonMaxima(const cv::Mat& in, cv::Mat& out, int neighborhood)
{
    ...
    for(p.y = 1; p.y < in.rows - 1; p.y++) {
        for(p.x = 1; p.x < in.cols - 1; p.x++) {
            isMax = true;
            for(i = 0; i < neighbors.size() && isMax; i++) isMax = (in.at<float>(p + neighbors[i]) <= in.at<float>(p));
            if(isMax) out.at<float>(p) = in.at<float>(p);
        }
    }
}
...
void localExtrema(const cv::Mat& in, cv::Mat& out, int neighborhood)
{
    ...
    for(p.y = 1; p.y < in.rows - 1; p.y++) {
        for(p.x = 1; p.x < in.cols - 1; p.x++) {
            isMin = true;
            for(i = 0; i < neighbors.size() && isMin; i++) isMin = (in.at<float>(p + neighbors[i]) > in.at<float>(p));
            if(isMin)
                out.at<uchar>(p) = 1;
            else {
                isMax = true;
