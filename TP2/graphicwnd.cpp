...
{
    if (pParentWnd!=NULL)
        pParentWnd->handleMousePress(event->x(), event->y(), event->button());
}

void CGraphicWndFrame::mouseReleaseEvent(QMouseEvent *event)
{
    if (pParentWnd!=NULL)
        pParentWnd->handleMouseRelease(event->x(), event->y(), event->button());
}

void CGraphicWndFrame::mouseMoveEvent(QMouseEvent *event)
{
    if (pParentWnd!=NULL)
        pParentWnd->handleMouseMove(event->x(), event->y());
}
...
void CGraphicWndFrame::paintEvent(QPaintEvent *)
{
    ...
    if (pParentWnd!=NULL)
        painter.drawImage(0, 0, pParentWnd->qimageZoom);
}
...
string CImage2DWnd::GetStatusBarString() const
{
    ...
    if (piCenter.inside(r))
    {
        switch (imgInput.type())
        {
            // Scalar types
            case CV_8U:
                streamStatus<<"  Value="<<(int)imgInput.at<unsigned char>(piCenter)<<" (uchar)"; break;
            case CV_32S:
                streamStatus<<"  Value="<<imgInput.at<int>(piCenter)<<" (int)"; break;
            case CV_32F:
                streamStatus<<"  Value="<<imgInput.at<float>(piCenter)<<" (float)"; break;

            // Vector types
            case CV_8UC3:
                vb = imgInput.at<cv::Vec3b>(piCenter);
                streamStatus<<"  [B,G,R]="<<vb<<" (3 x uchar)"; break;
            case CV_32SC3:
                vi = imgInput.at<cv::Vec3i>(piCenter);
                streamStatus<<"  [B,G,R]=("<<vi<<") (3 x int)"; break;
            case CV_32FC3:
                vf = imgInput.at<cv::Vec3f>(piCenter);
                streamStatus<<"  [B,G,R]="<<vf<<" (3 x float)"; break;
            default:
...
void CImage2DWnd::keyPressEvent(QKeyEvent *event)
{
    if (event->key()==Qt::Key_Space)
    {
        ...
        if (!fileName.isEmpty())
            SaveQImage(fileName.toLocal8Bit().constData());
    }
        ...
        if (dlg.result()==QDialog::Accepted)
        {
            if (imgInput.type()==CV_32S || imgInput.type()==CV_32F)
            {
                ...
                if (iNbValuesNew<listValuesColors.size())
                {
                    ...
                    for (iValue=0; iValue<iNbToRemove; iValue++)
                        listValuesColors.pop_back();
                }
                else if (iNbValuesNew>listValuesColors.size())
                {
                    ...
                    for (iValue=0; iValue<iNbToAdd; iValue++)
                        listValuesColors.push_back(valueAndColor);
                }
            }
            else if (imgInput.type()==CV_32SC3 || imgInput.type()==CV_32FC3)
	...
	for (p.y=0; p.y<height; p.y++)
	{
		...
		for (p.x=0; p.x<width; p.x++)
		{
		    switch (imgInput.type())
		    {
                ...
                case CV_8U: b = cv::Vec3b(*pArray, *pArray, *pArray); break;
                case CV_32S:
                    fValue = (float)(*((int *)pArray));
                    b = GetColorFromLUT(fValue); break;
                case CV_32F:
                    fValue = *((float *)pArray);
                    b = GetColorFromLUT(fValue); break;
                ...
                case CV_8UC3:
                    b = *((cv::Vec3b *)pArray); break;
                case CV_32FC3:
                    f = *((cv::Vec3f *)pArray);
                    b = VecOp::mul(VecOp::div((f-fMin),(fMax-fMin)),cv::Vec3f(255.0, 255.0, 255.0)); break;
                default:
...
cv::Vec3b CImage2DWnd::GetColorFromLUT(float fValue) const
{
    ...
    iColor = 0;
    while (iColor<listValuesColors.size() && fValue>listValuesColors[iColor].first)
        iColor++;
    ...
    if (iColor==0)
        ...
        // Interpolate color
        fCoef = (fValue-listValuesColors[iColor-1].first)/(listValuesColors[iColor].first - listValuesColors[iColor-1].first);
