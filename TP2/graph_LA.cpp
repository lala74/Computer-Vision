...
void GraphFlow::FordFulkerson()
{
    ...
    do {
        ...
        if (bPathFound)
        {
            ...
            // Add code here...
            float cap_arc = 0;          // capacité d'un arc
            float flow_arc = 0;         // écoulement d'un arc
            float res_cap_arc = 0;      // valeur de la capacité résiduelle
            float min_res_cap = pCurrentNode->pAugPathArc->capacity;    // valeur minimale du chemin d'augmentation

//            cout<<"Do 1st backtrack"<<endl;

            do {
                cap_arc = pCurrentNode->pAugPathArc->capacity;
                flow_arc = pCurrentNode->pAugPathArc->flow;
                res_cap_arc =  cap_arc - flow_arc;                      // calcul de la capacité résiduelle
                if (res_cap_arc < min_res_cap) {
                // si la capacité résiduelle est plus petite que la valeur minimale
                    min_res_cap = res_cap_arc;                          // on prends cette capacité résiduelle comme la valeur minimale
                }
                pCurrentNode = pCurrentNode->pAugPathArc->pStartNode;   // passer au noeud derrière
            } while (pCurrentNode != &nodes[0]);                        // on réalise le backtrack jusqu'au noeud source

//            cout<<"Do 2nd backtrack"<<endl;

            pCurrentNode = &nodes[1];                                   // on commence au noeud puits
            do {
                pCurrentNode->pAugPathArc->flow += min_res_cap;         // augmente l'écoulement de chaque arc du chemin d'augmentation
                pCurrentNode = pCurrentNode->pAugPathArc->pStartNode;   // passer au noeud derrière
